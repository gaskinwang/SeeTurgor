...
%Data Preprocessing: Get all the boundaries of each cell
clear;close all;
load Data2.mat
load Centre.mat
load Bound4Cell.mat
Data=xlsread('WT98 1+ 0.5 modelling data.xlsx');
CellAmount=size(Bound4Cell,1);
CellVertex=Bound4Cell;%Store vertex information for each cell



BoundaryCell=1:13;% Numbers of L1 cells
BoundaryPos=zeros(length(BoundaryCell),2);
Path=zeros(10000,size(Data,2));%All paths
Path_mark=1;
PathCount=zeros(10000,1);

% To facilitate debugging, temporarily introduce the visual part
% hold on
% for ii=1:CellAmount
...
for bc=BoundaryCell
    disp(bc)
    %Calculate the appropriate electrical probe insertion points
    Left=zeros(1,2);
    Right=zeros(1,2);
    ...
    for bouMark=length(boux)%For discrete points on L1 outer walls
        for Scan=0.01:ScanStep:2*pi%For each scan angle
            ...
            %Finding detected (i.e., inserted) cells
            if NowPathMark>size(Data,2)
                Exist=false;
...
%The basic part is completed, followed by introducing the framework of the genetic algorithm
AddationImformation.Path=Path;
AddationImformation.PathCount=PathCount;


% Here is a summary based on experimental measurement data (i.e., wt98 1+ 0.5 modelling data.xlsx)
Data(Data==0.5)=4;
